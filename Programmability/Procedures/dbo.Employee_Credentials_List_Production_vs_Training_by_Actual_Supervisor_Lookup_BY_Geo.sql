...
CREATE PROCEDURE [dbo].[Employee_Credentials_List_Production_vs_Training_by_Actual_Supervisor_Lookup_BY_Geo]

AS
SELECT DISTINCT (GEOAREANAME) FROM 
(
(
...
WHERE te.email NOT LIKE '%cred%' AND te.emp_status <> 'INACTIVE' AND ega.geoareas_id IS NOT NULL AND ga.geo_area <> '0'
...
WHERE e.email NOT LIKE '%cred%' AND e.emp_status <> 'INACTIVE' AND ega.geoareas_id IS NOT NULL AND ga.geo_area <> '0'
)
)
 AS INFO
