...
GO
CREATE PROCEDURE [dbo].[Insert_ApplicationRecord_Version_9]
...
@sApplicantGUID varchar(50)
		...
		AS 	-- modified 11/28/2007 by FrankDeMarco to add VulnerabilityAssessment
			...
			-- modified this for use in Johnet for Credible -> Johnet -> Student_Database update
	...
	IF(@sCopyLast = 'Yes')
		BEGIN			
			...
			SELECT @sNewGUID,@sApplicantGUID,  @sApplicationStatus, @dtCreated, AddressAtTimeOfApp, CityAtTimeOfApp, StateAtTimeOfApp, ZipAtTimeOfApp
				, CountyAtTimeOfApp,  PhoneAtTimeOfApp,1,@iArea
	...
	ELSE
		BEGIN
			...
			INSERT INTO [Student Database_KioskApp].dbo.Applications (ApplicationGUID,ApplicantGUID,ApplicationStatus,DateCreated,ApplicationRecordCopiedFromPriorRecord,Area)
			VALUES(@sNewGUID,@sApplicantGUID,@sApplicationStatus,@dtCreated,0,@iArea)
