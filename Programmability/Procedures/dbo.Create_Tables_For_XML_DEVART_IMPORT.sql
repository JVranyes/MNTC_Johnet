...
SELECT TABLE_NAME
      ...
      ,Column1
      ,Column2
      ,(lead(TABLE_NAME,1) OVER (ORDER BY TABLE_NAME,COLUMN_NAME))
      ,(LAG(TABLE_NAME,1) OVER (ORDER BY TABLE_NAME,COLUMN_NAME))

      FROM Johnet.dbo.Credible_Database_Schema_Columns_DataTypesSizes
WHERE TABLE_NAME NOT LIKE '%ZFORM%' AND CHARACTER_MAXIMUM_LENGTH NOT LIKE '%[^0-9]%'
...
SELECT @sTABLE_NAME = TABLE_NAME, @sCOLUMN_NAME =  COLUMN_NAME, @sIS_NULLABLE  =  IS_NULLABLE, @sDATA_TYPE = DATA_TYPE, @sCHARACTER_MAXIMUM_LENGTH = CHARACTER_MAXIUM_LENGTH, @sTABLE_CATALOG = TABLE_CATALOG, @sTABLE_SCHEMA = TABLE_SCHEMA, @sORDINAL_POSITION = ORDINAL_POSITION, @sDATETIME_PRECISION = DATETIME_PRECISON, @sColumn1 = Column1, @sColumn2 = Column2, @sNTABLE_NAME = NTABLE_NAME, @sPTABLE_NAME = PTABLE_NAME FROM @Enumerator
ORDER BY TABLE_NAME
...
set @later = current_timestamp+'00:00:00.05'
...
SET @sCHARACTER_MAXIMUM_LENGTH_FIXED = '(' + REPLACE(@sCHARACTER_MAXIMUM_LENGTH,'-1','max') + ')'
...
IF @sqlTableColumnCreate2 NOT LIKE '%'+@sCOLUMN_NAME+'%'
BEGIN
