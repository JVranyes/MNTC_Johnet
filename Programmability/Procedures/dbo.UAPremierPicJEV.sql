...
CREATE PROCEDURE [dbo].[UAPremierPicJEV]
...
LEFT JOIN ClientVisit cv ON sva.clientvisit_id = cv.clientvisit_id

LEFT JOIN Employees e ON cv.emp_id = e.emp_id
LEFT JOIN Clients c2 ON cv.client_id = c2.client_id
...
LEFT JOIN Team t ON cv.team_id = t.team_id
...
AND l.location_id = @UALookupID AND cv.appr = 1 AND cvi.billing_sequence = 1 
